...
import 'pditime_n_e_w_model.dart';
export 'pditime_n_e_w_model.dart';

class PditimeNEWWidget extends StatefulWidget {
  const PditimeNEWWidget({
    Key? key,
    this.modal,
  }) : super(key: key);
  ...
  @override
  _PditimeNEWWidgetState createState() => _PditimeNEWWidgetState();
}

class _PditimeNEWWidgetState extends State<PditimeNEWWidget>
    with TickerProviderStateMixin {
  late PditimeNEWModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => PditimeNEWModel());
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      if ((_model.apiToken?.succeeded ?? true)) {
        if ((FFAppState().isManager == true) || (FFAppState().userid == 1980)) {
          _model.apiResultPDITime = await PdiGroup.listPDICall.call(
            id: 0,
            collaboratorId: 0,
            managerId: FFAppState().userid,
          );
          if ((_model.apiResultPDITime?.jsonBody ?? '')) {
            FFAppState().update(() {
              FFAppState().myTemPdiJson =
                  (_model.apiResultPDITime?.jsonBody ?? '')
                      .toList()
                      .cast<dynamic>();
            });
          }
